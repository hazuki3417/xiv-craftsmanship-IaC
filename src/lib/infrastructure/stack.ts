...
export class Infrastructure extends cdk.Stack {
	constructor(scope: Construct, id: string, props: cdk.StackProps) {
		...
		vpc.addInterfaceEndpoint(
			name.ec2.vpc.interface.cloudwatchLogs.resource.id,
			{
		...
		new cdk.CfnOutput(this, name.ec2.sg.cfn.securityGroupId.exportId, {
			value: sgApp.securityGroupId,
			exportName: name.ec2.sg.cfn.securityGroupId.exportName,
		});

		new cdk.CfnOutput(this, name.ecs.cluster.cfn.arn.exportId, {
			value: cluster.clusterArn,
			exportName: name.ecs.cluster.cfn.arn.exportName,
		});
	}
}
